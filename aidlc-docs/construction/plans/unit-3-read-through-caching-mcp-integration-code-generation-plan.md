...
- [ ] **Step 1 — Config cleanup (FR-5.2)**: remove `CacheTTL`/
...
- [ ] **Step 2 — `CachingClient` core** (`internal/opcua/caching_client.go`,
...
- [ ] **Step 3 — `CachingClient` unit tests**
...
- [ ] **Step 4 — `CachingClient` TTL-invariant property test**
      (`internal/opcua/caching_client_pbt_test.go`, new): NFR-3.2 —
      `cached ⟺ within TTL` across randomly generated `ReceivedAt`/
      `maxAgeMs` pairs.

- [ ] **Step 5 — `mcp.Server` struct + constructor** (`internal/mcp/server.go`):
...
- [ ] **Step 6 — 3 new MCP tools** (`internal/mcp/server.go`):
...
- [ ] **Step 7 — `opcua_read`/`opcua_write`/`opcua_browse_nodes` integration**:
...
- [ ] **Step 8 — `mcp.Server` shutdown ordering** (`internal/mcp/server.go`):
...
- [ ] **Step 9 — `cmd/opcua-mcp.go` wiring** (BR-10/BR-11): `store.Open`
...
- [ ] **Step 10 — `internal/mcp/server_test.go` updates**: adjust every
...
- [ ] **Step 11 — Integration test suite** (NFR-4.1/4.2,
...
- [ ] **Step 12 — `Makefile` `test-integration` target**: add
...
- [ ] **Step 13 — `README.md` minimal additive update** (NFR-5.1): a
...
- [ ] **Step 14 — Documentation summary**: create
