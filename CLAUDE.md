...
Everything below reflects this repo's *current, verified* state after (1) a completed hardening pass over critical correctness (data races, stdio log corruption, wire-type decoding, input validation) and performance/robustness (partial-batch reads, browse pagination, discovery cache correctness, graceful shutdown) issues, and (2) three AIDLC construction units that have since landed in full: Unit 1 (persistent bbolt-backed store), Unit 2 (subscription management with reconnect handling), and Unit 3 (read-through caching decorator + MCP integration) - plus telemetry, added independently of those units. None of this is aspirational; it's all merged. The ad-hoc audit docs that drove the original hardening pass (`docs/plan/findings.md`/`plan.md`/`decisions.md`) have been retired now that their items have landed; `docs/COMMIT_CONVENTION.md` (this repo's commit message convention) is the one doc from that effort still in `docs/`. Further work is planned and tracked through the AIDLC workflow described at the top of this file - see "Where to look for current work" below.

### What this is

`opcua-mcp` is a Go MCP (Model Context Protocol) server that bridges LLM clients to OPC-UA industrial automation servers. It exposes 18 OPC-UA read/write/browse/discovery/subscribe operations as MCP tools over stdio or HTTP transport, maintains a searchable (Bleve full-text) cache of the server's address space via background discovery, and persists a read-through values/typeinfo/browse cache plus subscription intent to an on-disk bbolt store so both survive a restart.

### Architecture

- `cmd/opcua-mcp.go` - entrypoint. Loads config, initializes the logger, constructs `internal/opcua.Client`, connects eagerly unless `SERVER_TRANSPORT=stdio` (stdio connects lazily via the `opcua_connect` tool), opens the `internal/store.Store`, wires a `CachingClient` decorator and a `SubscriptionManager` around the client, constructs `internal/telemetry`, constructs `internal/mcp.Server`, starts it.
- `internal/mcp/server.go` - `Server` type. Registers all 18 MCP tools in `SetupTools()` (none removed or gated behind a flag yet), holds the `*opcua.Client`/`*opcua.CachingClient`/`*opcua.SubscriptionManager`/`*opcua.DiscoveryService`, runs the stdio or HTTP transport loop, handles graceful shutdown (`setupGracefulShutdown`) - which now also drains the running `*server.StreamableHTTPServer` (stored on `Server`, guarded by `httpServerMu`) before exiting in http mode. `internal/mcp/telemetry_middleware.go`'s `telemetryMiddleware` is registered once via `server.WithToolHandlerMiddleware`, giving every tool `RecordToolCall`/`RecordError` coverage from one place instead of instrumenting each `handleX` method; `classifyErrorText` reduces an error's message to one of `telemetry`'s fixed `ErrKind*` categories before anything is recorded.
...
- `internal/opcua/caching_client.go` - `CachingClient` type. A read-through caching decorator wrapping `*Client` from the outside (`Client` itself stays a pure protocol wrapper, unaware caching exists). Read/Browse/GetNodeTypeInfo check the store first (values/browse/typeinfo buckets respectively) within their configured TTL (`STORE_TYPEINFO_TTL`/`STORE_BROWSE_TTL`; browse and typeinfo use a plain age check, values additionally trust a `Source: "subscription"` entry unconditionally since a live subscription push is definitionally fresh) and fall back to `*Client` on a miss or stale entry, opportunistically writing the live result back. `Write` delegates to `*Client.Write` then invalidates the node's cached value unless it's currently subscribed (the next subscription push corrects it naturally). Every cache read/write failure is logged and falls through to live rather than failing the request - the cache degrades, it never breaks correctness. `SearchConfig.EnableCache` is the master on/off switch (`false` reproduces pre-Unit-3 always-live behavior exactly).
- `internal/opcua/subscription.go` - `SubscriptionManager` type. Manages OPC-UA subscription lifecycle: `Subscribe`/`Unsubscribe`/`ListSubscriptions`, grouping every logical subscription sharing an interval into one shared `gopcua` `Subscription` (`intervalGroup`), and persists subscription *intent* (node IDs + interval, never server-assigned ephemeral IDs, which are meaningless after a reconnect) to the store's subscriptions bucket. `Start` warm-starts every persisted subscription before returning, so `ListSubscriptions` is always consistent with persisted intent. A `pump` goroutine batches incoming notifications into values-bucket writes (tagged `Source: "subscription"`) within `STORE_BATCH_WINDOW`/`STORE_BATCH_MAX_ITEMS` bounds. `Subscribe` succeeds partially: nodes the server rejects are reported in `rejected`, never silently dropped.
- `internal/opcua/reconnect_watcher.go` - `ReconnectWatcher` type. Watches `Client`'s connection state and reacts *only* to a permanent death (a transition to `Closed` after having been `Connected`) - it deliberately ignores `Connecting`/`Reconnecting`/transient `Disconnected` states, since gopcua's own `AutoReconnect` already keeps existing subscriptions working across those. On a permanent death it calls `SubscriptionManager.rebuild`, which reconnects, discards all in-memory subscription/interval-group state (it died with the old client), and warm-starts everything again from persisted intent.
...
- `internal/store/` - `Store` type (`store.go`/`types.go`/`value_encoding.go`). A bbolt-backed key-value store with four buckets (values, typeinfo, browse, subscriptions) and no TTL/expiry logic of its own - it returns whatever was last written, verbatim; freshness decisions belong entirely to callers (`CachingClient`, `SubscriptionManager`). Relies solely on bbolt's own single-writer/MVCC-reader transaction model for concurrency safety, no additional locking layered on top. `Open` requires a non-zero timeout (`STORE_OPEN_TIMEOUT`), since bbolt retries its file lock every 50ms forever at `Timeout: 0` - a stale lock from a prior ungraceful shutdown would otherwise hang stdio startup indefinitely. `value_encoding.go`'s `encode`/`decode` preserve a cached value's exact Go dynamic type (int32 stays int32, not float64) across the JSON round-trip via an explicit `valueKind` tag, since `encoding/json`'s default `interface{}` unmarshaling collapses every number to `float64`.
- `internal/telemetry/` - anonymous, aggregate, opt-out product-usage telemetry sent to Aptabase, on by default (`DO_NOT_TRACK=1` or `OPCUA_MCP_TELEMETRY=false` to disable). One `session_summary` HTTP event per flush interval (default 10 min, `TELEMETRY_FLUSH_INTERVAL`), never one per tool call. The `Telemetry` interface's `noopTelemetry` implementation (returned by `New` when disabled) makes every call site a cheap no-op with no `if enabled` checks scattered through the codebase. `buildPayload` emits per-tool and per-error-kind counts as individually-named scalar props (`tool_calls.<name>`, `errors.<kind>`) rather than a single nested-object prop - Aptabase's dashboard renders a nested object value as an opaque, non-expandable "{Object}" chip, so flattening is what makes the breakdown actually visible in the Session Timeline view. The exact allowed data model (tool-call counts, cache hit/miss counts, coarse error categories, discovered-node-count buckets, auth mode/security policy *names* only) is documented at the top of `telemetry.go` and enforced by `TestSessionSummaryPayloadAllowlist` in `telemetry_test.go`, which validates `tool_calls.*`/`errors.*` by prefix against `knownToolNames`/`knownErrorKinds` (they're dynamic keys, not a fixed set) and every other prop by exact match - that test fails if a field outside the documented list is ever added to the outbound payload. Never collected, under any circumstance: node IDs, browse names, endpoint URLs/hostnames/IPs, any value read or written, raw error text, credentials.
- `internal/config/` - config via `spf13/viper`, five structs (`ServerConfig`/`SERVER_*`, `OPCUAConfig`/`OPCUA_*`, `MCPConfig`/`MCP_*`, `SearchConfig`/`SEARCH_*`, `StoreConfig`/`STORE_*`), loaded once in `cmd/opcua-mcp.go`. Each field carries a `mapstructure:"..."` tag (snake_case, no prefix) rather than the old `env`/`envDefault` tags; defaults are registered explicitly in `setDefaults()` rather than via struct tags, and must stay in sync with it. Precedence is defaults < optional config file (TOML/YAML/JSON/etc, `./config.*` or `CONFIG_FILE=<path>`) < environment variables - env vars always win, preserving every existing `SERVER_*`/`OPCUA_*`/`MCP_*`/`SEARCH_*`/`STORE_*` name as-is. `Load()` builds a fresh `viper.New()` instance per call rather than using viper's global instance, so tests that set/unset env vars don't leak state into each other. `internal/telemetry.Config` deliberately does *not* route through this struct - see its own doc comment for why.
...
- **No new direct dependencies without explicit sign-off.** `go.etcd.io/bbolt` has already been promoted from an indirect Bleve dependency to a direct one for `internal/store` - that promotion is done, not pending. Any *further* new direct dependency still needs explicit sign-off.
- **Preserve env var and tool-name backward compatibility.** All 18 currently-registered tools are still present and ungated; any removal or gating (e.g. behind a feature flag) needs explicit sign-off and a README changelog callout, same bar as the `opcua_write`/stdio-logging behavior changes already called out there.
...
- **Don't call `store`/Bleve methods while holding `DiscoveryService.cacheMutex` or `SubscriptionManager.mu`** - snapshot needed data under the lock, release, then do I/O. `discoveryMu` is held for a whole walk but never across a Bleve I/O call outside `updateSearchIndex`'s existing batch pattern; `SubscriptionManager`'s `pump` similarly writes to the store outside its own lock scope - keep both that way.
- **Never widen `internal/telemetry`'s outbound payload without updating `TestSessionSummaryPayloadAllowlist`.** Any new field added to `buildPayload` in `telemetry.go` must be added to the allowlist test in the same change, and must not be (or be derived from) a node ID, browse name, endpoint URL/hostname/IP, a value read or written, raw error text, or a credential - see the doc comment at the top of `telemetry.go` for the full collected/never-collected lists. A new *fixed-name* prop goes in `allowedPropsKeys`; a new *dynamic-key* breakdown (like `tool_calls.<name>`/`errors.<kind>`) needs its own prefix branch validating the suffix against a known-values set, not a literal allowlist entry.
...
- The `opcuaClient` mock-seam pattern is followed consistently for every other collaborator that needs a test double: `subscribingClient`/`SubscriptionHandle` (`internal/opcua/subscription.go`, mocked in `mock_subscription_test.go`) and `valueTypeBrowseStore`/`subscriptionStore` (narrow slices of `*store.Store`'s surface, defined next to their consumer). Each interface exposes only the methods its specific consumer needs - don't grow one to cover a future consumer speculatively; define a new narrow interface instead.
